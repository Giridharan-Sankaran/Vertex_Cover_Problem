...
def printoutput(vertex,edges):
    ...
    k=1
    for l in vertex:
        print("{0} : ({1},{2})".format(k,float(l[0]) ,float(l[1]) ),file=sys.stdout) 
        k+=1
        outputprint[k]=l;
    print("}",file=sys.stdout)
    key_list = list(outputprint.keys()) 
    val_list = list(outputprint.values())
    print("E = {",file=sys.stdout)
    i = 0
    for b in edges:
        if i == len(edges) -1:
            print("<{0},{1}>".format(val_list.index(b[0])+1 , val_list.index(b[1])+1),file=sys.stdout)
            break
        print("<{0},{1}>".format(val_list.index(b[0])+1 , val_list.index(b[1])+1)+",",file=sys.stdout)
        i += 1
    print("}",file=sys.stdout)
